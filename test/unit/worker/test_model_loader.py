...
from vllm_neuron.worker.neuronx_distributed_model_loader import (
    ...
    _validate_override_neuron_config, get_neuron_model)
...
def test_neuron_pixtral_raises_error_when_batch_size_exceeds_limit(mocker):
    ...
    mock_neuron_config_cls.side_effect = [
        mock_vision_neuron_config, mock_text_neuron_config
    ]
    ...
    with pytest.raises(
            ValueError,
            match=
            "Neuron Pixtral model does not support batch size > 4 in vLLM v1 yet."
    ):
        model.load_weights("model_path", "LlavaForConditionalGeneration",
                           **kwargs)
