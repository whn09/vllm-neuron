...
class TestModelRunnerValidation:
    ...
    def mock_vllm_config(self):
        ...
        config.scheduler_config = Mock(
            max_model_len=2048, max_num_seqs=32, max_num_batched_tokens=4096
        )
    ...
    def model_runner(self, mock_vllm_config):
        ...
        runner = NeuronxDistributedModelRunner(
            vllm_config=mock_vllm_config, device="cpu"
        )
    ...
    def test_validate_max_prompt_length_config_match_success(self, model_runner):
    ...
    def test_validate_max_prompt_length_config_mismatch_error(self, model_runner):
        ...
        assert (
            "does not match the Neuron model's compiled max prompt length (1024)"
            in error_msg
        )
        assert (
            'Please update --additional-config to set "max_prompt_length": 1024'
            in error_msg
        )
    ...
    def test_validate_max_prompt_length_no_user_config_warning(
        ...
        assert (
            "Your Neuron model was compiled with max prompt length 1024" in warning_msg
        )
    ...
    def test_runtime_validation_within_limits(self, model_runner):
        ...
        assert len(request_data.prompt_token_ids) <= model_runner.max_prompt_length
    ...
    def test_runtime_validation_exceeds_limits(self, model_runner):
        ...
        error_msg = str(exc_info.value)
        assert (
            "Prompt length (12 tokens) exceeds the maximum prompt length (10 tokens)"
            in error_msg
        )
        ...
        assert (
            "This will return a 400 error instead of terminating the engine"
            in error_msg
        )
        assert (
            "supported on OpenAI /v1/completions and /v1/chat/completions endpoints"
            in error_msg
        )
    ...
    def test_warning_message_formatting(self, model_runner, caplog):
        ...
        assert (
            "Your Neuron model was compiled with max_prompt_length=1024" in warning_msg
        )
        assert "but max_model_len is set to 2048" in warning_msg
        assert "To prevent the vLLM engine from crashing" in warning_msg
    ...
    def test_validation_boundary_conditions(
        self, model_runner, prompt_length, max_length, should_pass
    ):
        ...
        if should_pass:
            ...
            assert f"maximum prompt length ({max_length} tokens)" in str(exc_info.value)
    ...
    def test_validation_with_none_max_prompt_length(self, model_runner):
        ...
        def validate_prompt_length(token_count):
            if (
                model_runner.max_prompt_length is not None
                and token_count > model_runner.max_prompt_length
            ):
    ...
    def test_error_message_consistency_across_scenarios(self, model_runner):
        ...
        for scenario in scenarios:
            ...
            for phrase in scenario["key_phrases"]:
                assert phrase in error_msg, (
                    f"Missing phrase '{phrase}' in error: {error_msg}"
                )
