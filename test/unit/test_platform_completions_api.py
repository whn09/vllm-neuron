...
class TestCompletionsAPIValidation:
    ...
    def test_completion_renderer_validation_within_limits(self):
        ...
        def neuron_create_tokens_prompt(
            renderer, token_ids, max_length=None, cache_salt=None, prompt=None
        ):
    ...
    def test_completion_renderer_validation_exceeds_limits(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            neuron_create_tokens_prompt(mock_renderer, [1, 2, 3, 4, 5, 6, 7, 8])
    ...
    def test_openai_serving_validation_exceeds_limits(self):
        ...
        # Test with input exceeding limits
        with pytest.raises(ValueError) as exc_info:
            neuron_validate_input(
                mock_serving, Mock(), [1, 2, 3, 4, 5, 6, 7, 8], "test"
            )
    ...
    def test_openai_serving_validation_none_max_prompt_length(self):
        ...
        # Test with large input - should not raise error
        result = neuron_validate_input(mock_serving, Mock(), [1] * 1000, "test")
    ...
    def test_validation_boundary_conditions(
        self, input_length, max_length, should_pass
    ):
    ...
    def test_dual_endpoint_validation_consistency(self):
        ...
        def completions_validate(token_ids, max_prompt_length):
            ...
            if max_prompt_length is not None and len(token_ids) > max_prompt_length:
                return (
                    False,
                    f"Completions: Prompt length {len(token_ids)} exceeds {max_prompt_length}",
                )
        ...
        def chat_completions_validate(token_ids, max_prompt_length):
            ...
            if max_prompt_length is not None and len(token_ids) > max_prompt_length:
                return (
                    False,
                    f"Chat: Prompt length {len(token_ids)} exceeds {max_prompt_length}",
                )
    ...
    def test_validation_with_various_token_types(self):
        ...
        def validate_input(token_ids, max_length):
            if max_length is not None and len(token_ids) > max_length:
                raise ValueError(f"Too many tokens: {len(token_ids)} > {max_length}")
